...
INSTALLED_APPS=['django.contrib.admin','django.contrib.auth','django.contrib.contenttypes','django.contrib.sessions','django.contrib.messages','django.contrib.staticfiles','corsheaders','rest_framework','rest_framework_simplejwt','apps.accounts','apps.training','apps.gamification']
...
FRONTEND_URL=os.getenv('FRONTEND_URL','https://cyberguard-cybersecurity-training-git-main-wiltech.vercel.app')
CORS_ALLOWED_ORIGINS=list(dict.fromkeys([
    x.strip()
    for x in os.getenv('CORS_ALLOWED_ORIGINS', FRONTEND_URL + ',http://localhost:3000').split(',')
    if x.strip()
]))
CSRF_TRUSTED_ORIGINS=[x for x in CORS_ALLOWED_ORIGINS if x.startswith('https://')]
